...
const sort = (arr) => {
  return arr.sort((a,b)=>a-b)// return no. not string => 1,11,2,3,4
}

let result = median ([2,3,1,11,4,5,6])
console.log(result);
...
```js
function myRotate(arr, shift = 1) {
  if (shift > 0) { // pop and shift => length NOT MATTER
    for (let i = 0; i < shift % arr.length; i++) { // til the end of the shifting process
      let shifted = arr.shift(); // console.log(arr.shift());
      arr.push(shifted);
    }
  } else {
